...
run.mcmc <- function(banditarm, totaliterations,set.beta){
  ...
  for(i in 1:N){
    for(j in 1:N){
      threshold1[i,j] <- ifelse(dataset$uniblock[i]==dataset$uniblock[j], 1 , jumpprob)
    }
  }
  ...
  for (m in 2:M){
    ...
    occult.prob<- as.numeric(as.character(occult/m))
