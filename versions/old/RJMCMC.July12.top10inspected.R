...
#ADD TOP 10 INSPECTED HOUSES AS NEGATIVE
top10unicodes <- c("1.24.5.250",
                   "1.24.5.228",
                   "1.24.5.264",
                   "1.24.5.263",
                   "1.24.5.227",
...
i.v.gps.456$ANIO[which(i.v.gps.456$UNICODE %in% top10unicodes)]=2015
#ADD TOP 10 INSPECTED HOUSES AS NEGATIVE
top10unicodes <- c("1.24.5.250",
                   "1.24.5.228",
                   "1.24.5.264",
...
i.v.gps.456$MES[which(i.v.gps.456$UNICODE %in% top10unicodes)]=7
i.v.gps.456$DIA[which(i.v.gps.456$UNICODE %in% top10unicodes)]=13
i.v.gps.456$ANIO[which(i.v.gps.456$UNICODE %in% top10unicodes)]=2015
...
for (m in 2:M){
  ...
  if(m%%5000==0) {write.table(occult.prob.ids.ordered, file=paste(m, "ResultsJuly11v2", sep=""))}
  if(m%%10==0) {print(m)}
}
