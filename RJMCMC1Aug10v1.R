...
#Houses within hop radius set as inspected
allhouses <- read.table("/home/data/Data/allhouses.aug24")
...
Results1 <- run.mcmc(1,1000000)
write.table(Results1, file="/home/data/Jewell/SensAnalysis/output/results.v1.withinradius", row.names=FALSE)



colfunc = gray.colors(length(unique(as.numeric(Results1[,2]))),start=1,end=0)[as.factor(Results1[,2])]
plot(Results1[,3], Results1[,4],col = colfunc,pch=16,cex=as.numeric(Results1[,2])*2000) #as.numeric(Results1[,2])*2000)
for (i in 1:867) if(sum.insp[i]>0) points(dataset$X[i],dataset$Y[i],pch=18,col="firebrick3",cex=1.5)
