...
inspecciones <- inspecciones[,c("UNICODE", "DIA", "MES", "ANIO", "PD_TCAP_TOT","IN_TCAP_TOT")]
...
#   if(m%%100000==0) {
#   	print(m)
#   	write.csv(occult.prob.ids.ordered, file=paste("beta",betastart,"ResultsSept13.csv", sep=""))
#     save.image(paste("beta",betastart,"ResultsSept13.Rdata", sep=""))}
if(m%%100==0){
  print(m)
  print(N_I)
  print(beta[m])
...
plot(dataset$X,dataset$Y,col="gray70",pch=16,xlab="",ylab="",xaxt="n",yaxt="n")
points(dataset$X,dataset$Y,col="skyblue4", pch=16, xlab="",ylab="",xaxt="n",yaxt="n",cex=1-inspected)
points(dataset$X,dataset$Y,col="firebrick4",pch=16,xlab="",ylab="",xaxt="n",yaxt="n",cex=positive)
legend("bottomleft",legend=c("Infested", "Uninfested", "Unobserved"), col=c("firebrick4", "gray70", "skyblue4"),pch=c(16,16,16),bty="n")
points(dataset$X, dataset$Y, cex=dataset$predicteddensity*30)
