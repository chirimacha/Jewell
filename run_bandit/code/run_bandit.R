...
if(Sys.getenv("SPATIAL_UNCERTAINTY") != "") {
  setwd(paste(Sys.getenv("SPATIAL_UNCERTAINTY"), "/run_bandit", sep=""))  
} else if(Sys.getenv("USER") == "sgutfraind") {
  setwd("/Users/sgutfraind/academic/chagas/bandit/run_bandit")
} else if(Sys.getenv("USER") == "sasha") {
  setwd("/home/sasha/Documents/projects/chagas/shared/Bandit/run_bandit")
  ...
  setwd("PATH_TO_BANDIT/run_bandit")
} else if(Sys.getenv("USER") == "javier") {
  setwd("/home/javier/Documentos/Mis Documentos/github/Bandit/Bandit/run_bandit")
...
bandit_initialize <- function(params) {
  ...
  if(is.null(params[["output_path"]])) {
    fpath <- time_stamp(paste("output/bandit_states/bandit_state_","run_",bandit.run,"_", sep=""), ".RSave")
  } else {
    fpath <- params[["output_path"]]
  }
  fpathcsv <-time_stamp(paste("output/bandit_states/bandit_csv/","run_",bandit.run, sep=""), ".csv")
...
#       recs[i,j] <-       
  ...
  save(bandit_state, file=fpath)
  write.csv(scores.pref,paste("output/bandit_csv/bandit_arm_results_","run_",bandit.run,"_", TimeNow(), ".csv", sep=""),row.names =FALSE)
  write.csv(all.recs,paste("output/bandit_csv/bandit_arm_recs_","run_",bandit.run,"_", TimeNow(), ".csv", sep=""),row.names =FALSE)
  cat("Saved bandit to:", fpath, "\n")
  return(fpath)
}
...
parse_cmdl <- function(params=def_params, alt_params=list()) {
  ...
  if (params$exec_mode == "update") {
    if (! is.null(opt$inspecciones_ruta)) {
      ...
      if (! is.null(opt$dia) & ! is.null(opt$ano) & !is.null(opt$mes)) {
        sel<-which(inspect$DIA==opt$dia & inspect$MES==opt$mes & inspect$ANIO)
        inspect<-inspect[sel,] #take only the data from the day
        
        #check to see if you've found any inspections 
        if (nrow(inspect)==0) {
          print("Warning: No data found for this day")
          q(1)
        }  
        cat("Found Inspections", nrow(inspect), "\n")
        
        
        inspect<-inspect[inspect$ETAPA==2,]
        inspect<-merge(inspect,assignment,by.x="UNICODE",by.y="unicode")
        inspect$IN_TCAP_TOT[is.na(inspect$IN_TCAP_TOT)]<-0
        ...
        print("Missing dia/mes/ano argument")
        q(1)
      }
    } else {
      #manual mode 
      print("Warning: entering manual mode")
