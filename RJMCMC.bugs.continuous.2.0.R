...
while(sum(indicator)<5 | indicator2<2){
  ...
  threshold=ifelse(distance<.05,1,.02)
  W=beta*threshold
...
tobs=90 #when observation occurs
m=1 #first iteration
...
for (m in 2:M){
  ...
  #####add in other part of likelhood####
  for (i in which(I!=Inf)){
...
for (i in 1:N) if(occult.sum.new[i]>0.03) points(location[i,1],location[i,2],pch=19,col="gold",xlab="X",ylab="Y",main = substitute(paste(Time, " = ", t),list(t=t)))
for (i in 1:N) if(occult.sum.new[i]<=0.03&occult.sum.new[i]>0.02) points(location[i,1],location[i,2],pch=19,col="darkolivegreen2",xlab="X",ylab="Y",main = substitute(paste(Time, " = ", t),list(t=t)))
for (i in 1:N) if(infectiontime[i]>=tobs) points(location[i,1],location[i,2],pch=1,col="firebrick1")
legend("topright", inset=c(-0.7,0),c("Observed Infested", "P(Infested)>0.03","0.02<P(Infested)<0.03","True Occult Infestation"),bty="n",col=c("firebrick4","gold","darkolivegreen1","firebrick1"),pch=c(18,19,19,1))
