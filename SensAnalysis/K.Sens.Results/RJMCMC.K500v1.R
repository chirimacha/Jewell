...
for (m in 2:M){
  ...
  if(m%%50000==0) {write.table(occult.prob.ids.ordered, file=paste("/home/ebillig/Jewell_data/SensAnalysis/",K,".",m, "ResultsKv1", sep=""))}
  if(m%%10000==0) {top.probs[,m/10000] <- occult.prob.ids.ordered[which(occult.prob.ids.ordered[,1] %in% top10),2]}
  if(m%%100000==0) {save.image(file = paste("/home/ebillig/Jewell_data/SensAnalysis/WorkSpace",K,".",m,"_Kv1.RData"))}
}
