...
#r <- get_my_reward(my_system,arm_idx)
#bandit <- update_bandit(bandit,arm_idx,r)
#cycle #2
#arm_idx <- next_arm(bandit)
#r <- get_my_reward(system,arm_idx)
#bandit <- update_bandit(bandit,arm_idx,r)
#...etc
...
#TODO: set this to your code
setwd("~/projects/chagas/code")
...
get_my_reward <- function(arm_idx) { 
  stopifnot(arm_idx >= 1)
...
zz_test_bandit_rc <- function() {
  ...
  for (trial in times) {
    ...
    reward     <- get_my_reward(chosen_arm)
...
zz_test_bandit_ucb1 <- function () {
  print("testing ucb1 bandit");
  times   <- seq(1,test_time)
  arms    <- rep(0,length(times))
  rewards <- rep(0,length(times))
  bandit <- initialize_ucb1(n_arms=n_arms, reward_func=get_my_reward)
  print(bandit)
  for (trial in times) {
    cat("trial: ", trial, "\n")
    chosen_arm <- next_arm(bandit)
...
zz_comparison_plot <- function(rc.results=NULL, ucb1.results=NULL) {
...
rc.results   <- zz_test_bandit_rc()
ucb1.results <- zz_test_bandit_ucb1()
browserxyz<-zz_comparison_plot(rc.results=rc.results, ucb1.results=ucb1.results)
