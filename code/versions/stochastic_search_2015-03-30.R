...
# )
  ...
  alg_params <- list(
    A=list(
      site_score = function(x) {jitter(x + rpois(length(x), lambda=1))},
      verbose=TRUE
    ),
    B=list(
      site_score = function(x) {jitter(x + rpois(length(x), lambda=5))},
  ...
  rewards <- rep(0,length(times))
  pA      <- rep(0,length(times))

  bandit <- initialize_rc(n_arms=2, discount_factor=0.0, learning_rate=0.1)
  print(bandit)
  ...
  results <- data.frame(times=times, ChosenArm=arms, Reward=rewards, CumulativeReward=cumsum(rewards),
                        pA=pA,  wins.by.A.timeline=wins.by.A.timeline)
  write.csv(results, paste("output/rc_results_", timeNow, ".csv", sep=""))
